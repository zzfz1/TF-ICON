...
def load_img(path, SCALE, pad=False, seg=False, target_size=None):
    if seg:
        ...
        segmentation_map_resize = cv2.resize(
            np.array(seg_map).astype(np.uint8),
            (new_w, new_h),
        ...
        padded_segmentation_map[
            start_x : start_x + segmentation_map_resize.shape[0],
            start_y : start_y + segmentation_map_resize.shape[1],
...
def main():
    ...
    parser.add_argument(
        "--ref-img", type=list, nargs="?", help="path to the input image"
    )
    ...
    for prompt_dir in os.listdir(opt.root):
        prompt_path = os.path.join(opt.root, prompt_dir)
        if not os.path.isdir(prompt_path):
            continue
            # Check if this directory contains set subdirectories
        subdirs = [
            d
            for d in os.listdir(prompt_path)
            if os.path.isdir(os.path.join(prompt_path, d)) and d.startswith("set")
        ]

        if subdirs:
            # Process each set subdirectory
            for set_dir in sorted(subdirs):
                set_path = os.path.join(prompt_path, set_dir)
                print(f"Processing {prompt_dir}/{set_dir}")
                process_image_set(
                    model,
                    sampler,
                    device,
            ...
            # Process the prompt directory directly (legacy format)
...
def process_image_set(
    model, sampler, device, opt, set_path, prompt, sample_path, base_count, set_name=""
):
    """Process a single set of images with the given prompt"""
    ...
    # Find all image files in the set directory
    for file in os.listdir(set_path):
        ...
        if file.startswith("bg"):
            init_img_path = file_path
        elif file.startswith("fg") and not (
            file.endswith("mask.jpg") or file.endswith("mask.png")
        ):
            ref_img_path = file_path
        elif file.startswith("mask"):
            mask_path = file_path
        elif file.startswith("fg") and (
            file.endswith("mask.jpg") or file.endswith("mask.png")
        ):
            seg_path = file_path

    # Verify all required files are found
    if not all([init_img_path, ref_img_path, mask_path, seg_path]):
        print(f"Warning: Missing required files in {set_path}")
        print(
            f"Found - bg: {init_img_path is not None}, fg: {ref_img_path is not None}, mask: {mask_path is not None}, seg: {seg_path is not None}"
        )
        return
    # Set the file paths in opt
    opt.init_img = init_img_path
    opt.ref_img = ref_img_path
    opt.mask = mask_path
    opt.seg = seg_path
    ...
    segmentation_map_orig = repeat(
        torch.tensor(segmentation_map)[None, None, ...].to(device),
        "1 1 ... -> b 4 ...",
        b=batch_size,
    )
    segmentation_map_save = repeat(
        torch.tensor(segmentation_map)[None, None, ...].to(device),
        "1 1 ... -> b 3 ...",
        b=batch_size,
    )
    ...
    top_rr = int(
        (0.5 * (target_height - height)) / target_height * init_image.shape[2]
    )  # xx% from the top
    bottom_rr = int(
        (0.5 * (target_height + height)) / target_height * init_image.shape[2]
    )
    left_rr = int(
        (0.5 * (target_width - width)) / target_width * init_image.shape[3]
    )  # xx% from the left
    ...
    image = Image.fromarray(
        ((save_image / torch.max(save_image.max(), abs(save_image.min())) + 1) * 127.5)[
            0
        ]
        .permute(1, 2, 0)
    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            for prompts in data:
                ...
                left_rr = int(
                    (0.5 * (target_width - width)) / target_width * init_latent.shape[3]
                )
                right_rr = int(
                    (0.5 * (target_width + width)) / target_width * init_latent.shape[3]
                )
                ...
                ref_latent = model.get_first_stage_encoding(
                    model.encode_first_stage(ref_image)
                )
                ...
                z_enc[:, :, param[0] : param[1], param[2] : param[3]] = z_enc[
                    ...
                    :, :, top_rr:bottom_rr, left_rr:right_rr
                ] + torch.randn(
                ...
                if opt.domain == "cross":
                    samples[:, :, param[0] : param[1], param[2] : param[3]] = (
                        torch.randn(
                            (1, 4, bottom_rr - top_rr, right_rr - left_rr),
                            device=device,
                        )
                    )
                ...
                for x_sample in x_samples:
                    ...
                    # Include set name in the filename for better organization
                    if set_name and set_name != "single":
                        filename = f"{base_count:05}_{set_name}_{prompts[0]}.png"
                    else:
                        filename = f"{base_count:05}_{prompts[0]}.png"
